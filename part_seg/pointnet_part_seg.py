...
def get_transform_K(inputs, is_training, bn_decay=None, K = 3):
    ...
    net = tf_util.fully_connected(net, 64, bn=True, is_training=is_training, scope='tfc1', bn_decay=bn_decay)
    net = tf_util.fully_connected(net, 64, bn=True, is_training=is_training, scope='tfc2', bn_decay=bn_decay)

    with tf.variable_scope('transform_feat') as sc:
        weights = tf.get_variable('weights', [64, K*K], initializer=tf.constant_initializer(0.0), dtype=tf.float32)
...
def get_transform(point_cloud, is_training, bn_decay=None, K = 3):
    ...
    net = tf.reshape(net, [batch_size, -1])
    net = tf_util.fully_connected(net, 32, bn=True, is_training=is_training, scope='tfc1', bn_decay=bn_decay)
    net = tf_util.fully_connected(net, 32, bn=True, is_training=is_training, scope='tfc2', bn_decay=bn_decay)

    with tf.variable_scope('transform_XYZ') as sc:
        assert(K==3)
        weights = tf.get_variable('weights', [32, 3*K], initializer=tf.constant_initializer(0.0), dtype=tf.float32)
...
def get_model(point_cloud, input_label, is_training, cat_num, part_num, \
    ...
    out2 = tf_util.conv2d(out1, 64, [1,1], padding='VALID', stride=[1,1],
                         bn=True, is_training=is_training, scope='conv2', bn_decay=bn_decay)
    out3 = tf_util.conv2d(out2, 64, [1,1], padding='VALID', stride=[1,1],
    ...
    squeezed_out3 = tf.reshape(out3, [batch_size, num_point, 64])
    ...
    out5 = tf_util.conv2d(out4, 512, [1,1], padding='VALID', stride=[1,1],
    ...
    net = tf_util.fully_connected(net, 64, bn=True, is_training=is_training, scope='cla/fc1', bn_decay=bn_decay)
    net = tf_util.fully_connected(net, 64, bn=True, is_training=is_training, scope='cla/fc2', bn_decay=bn_decay)
